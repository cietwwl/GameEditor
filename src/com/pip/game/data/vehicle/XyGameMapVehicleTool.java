...
import scryer.ogre.mesh.EntityAnimatePlayer;
...
public class XyGameMapVehicleTool implements IMapEditTool {
    ...
    // NPC模型
    private MeshConfig npcMesh;
    private EntityAnimatePlayer player;
    // 最近一次检测到的鼠标位置
    ...
    public XyGameMapVehicleTool(GameAreaEditor editor, GameMapViewer viewer, Vehicle template) {
        ...
        if(template.image != null){
            if(template.image instanceof Animation){
                npcAnimate = new PipAnimateSet();
                try {
                    npcAnimate.load(template.image.getAnimateFile(viewer.getMapFormat().aniFormat.id));
                } catch (Exception e) {
                }
            }else if(template.image instanceof GameMesh){
                npcMesh = MeshConfig.getBufferedMeshConfig(template.image.getAnimateFile(viewer.getMapFormat().aniFormat.id));
                        // MeshConfig.getResource(template.image.getAnimateFile(viewer.getMapFormat().aniFormat.id).getAbsolutePath());
//                npcMesh = new MeshConfig();
                try {
                    if(npcMesh != null){
                        npcMesh.getMesh();
                        player = new EntityAnimatePlayer("test");
                        player.init(npcMesh);
                        player.setDirection(0);
                        player.setShown(true);
                        player.setAnimation(0, -1);
                    }
//                    npcMesh.load(template.image.getAnimateFile(viewer.getMapFormat().aniFormat.id));
                } catch (Exception e) {
                }
            }
        }
    ...
    public void draw(GLGraphics gc) {
        ...
        if(npcAnimate != null){
         // 绘制NPC
            ...
            gc.drawText(coordStr, size.x - ts.x - 5, size.y - ts.y - 5);
        }else if(npcMesh != null){
//            npcMesh.drawInMap(gc);
            player.draw(gc.getHandle(), 0, 0);
        }
    }
