 ...
 * @author lighthu
 */
public class MultiTargetMapExitTool implements IMapEditTool {
    // 父编辑器
     ...
     * @param tv 贴图查看器
     */
    public MultiTargetMapExitTool(GameAreaEditor editor, GameMapViewer viewer) {
        this.editor = editor;
        this.viewer = viewer;
    }
    ...
    public void mouseUp(int x, int y, int mask) {
        ...
        if (isValidPos(pt)) {
            ...
            MultiTargetMapExit mexit = new MultiTargetMapExit();
            mexit.owner = mapInfo;
            mexit.id = 0;
            while (true) {
                // 确保ID不重复
                ...
                for (int i = mapInfo.objects.size() - 1; i >= 0; i--) {
                    if (mapInfo.objects.get(i).id == mexit.id) {
                        dup = true;
                        break;
                    }
                }
