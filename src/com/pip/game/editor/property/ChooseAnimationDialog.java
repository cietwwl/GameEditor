 ...
 * @author lighthu
 */
public class ChooseAnimationDialog extends Dialog {
    // 过滤条件
    private Text text;
    ...
    class TreeContentProvider implements IStructuredContentProvider, ITreeContentProvider {
        ...
        public Object[] getChildren(Object parentElement) {
            if (parentElement instanceof ProjectData) {
                ...
                List<DataObjectCategory> cateList = ((ProjectData) parentElement).getCategoryListByType(Animation.class);
    ...
    private Tree tree;
    private Animation selDataObject;
    ...
    public void setSelectedAnimation(int animationID) {
        selDataObject = (Animation)ProjectData.getActiveProject().findObject(Animation.class, animationID);
    }
     ...
     * @param parentShell
     */
    public ChooseAnimationDialog(Shell parentShell) {
        super(parentShell);
    }
    ...
    protected Control createDialogArea(Composite parent) {
        ...
        final Composite previewContainer = new Composite(container, SWT.NONE);
        previewContainer.setLayout(new FillLayout());
        final GridData gd_previewContainer = new GridData(SWT.FILL, SWT.FILL, true, true, 2, 1);
        previewContainer.setLayoutData(gd_previewContainer);

        previewer = new AnimatePreviewer(previewContainer, SWT.NONE);
    ...
    protected void configureShell(Shell newShell) {
        ...
        newShell.setText("选择动画");
    }

    protected void buttonPressed(int buttonId) {
        if (buttonId == IDialogConstants.OK_ID) {
            StructuredSelection sel = (StructuredSelection) treeViewer.getSelection();
            if (sel.isEmpty() || !(sel.getFirstElement() instanceof Animation)) {
                selDataObject = null;
            } else {
                selDataObject = (Animation)sel.getFirstElement();
            }
        }
    ...
    private void updatePreviewer() {
        ...
        if (sel.isEmpty() || !(sel.getFirstElement() instanceof Animation)) {
            ...
            Animation ani = (Animation)sel.getFirstElement();
            previewer.setAnimateFile(ani.getAnimateFile(0));
        }
    }
}
