...
public class ChooseBuffSetDialog extends Dialog {
     ...
     * @param parentShell
     */
    public ChooseBuffSetDialog(Shell parentShell) {
        super(parentShell);
    }
    ...
    protected Control createDialogArea(Composite parent) {
        ...
        List<BuffConfig> selObjs = new ArrayList<BuffConfig>();
        for (int id : selectedSkills) {
            try {
                BuffConfig q = (BuffConfig)ProjectData.getActiveProject().findObject(BuffConfig.class, id);
                if (q != null) {
                    selObjs.add(q);
                }
            } catch (Exception e) {
    ...
    protected void buttonPressed(int buttonId) {
        if (buttonId == IDialogConstants.OK_ID) {
            ...
            for (int i = 0; i < objs.length; i++) {
                selectedSkills[i] = ((BuffConfig)objs[i]).id;
            }
        }
