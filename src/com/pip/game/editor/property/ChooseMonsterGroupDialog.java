...
public class ChooseMonsterGroupDialog extends Dialog {
    class ListContentProvider implements IStructuredContentProvider {
        public Object[] getElements(Object inputElement) {
            List<DataObject> list = ((ProjectData)inputElement).getDataListByType(MonsterGroup.class);
            List<DataObject> retList = new ArrayList<DataObject>();
            for (int i = 0; i < list.size(); i++) {
                MonsterGroup t = (MonsterGroup)list.get(i);
                if (matchCondition(t)) {
                    retList.add(t);
                }
            }
    ...
    private boolean matchCondition(MonsterGroup t) {
     ...
     * @param parentShell
     */
    public ChooseMonsterGroupDialog(Shell parentShell) {
        super(parentShell);
    }
    ...
    protected Control createDialogArea(Composite parent) {
        ...
        if (selectedTemplate != -1) {
            try {
                // 查找这个NPC在tree中的位置
                MonsterGroup t = (MonsterGroup)ProjectData.getActiveProject().findObject(MonsterGroup.class, selectedTemplate);
                if (t != null) {
    ...
    protected void configureShell(Shell newShell) {
        ...
        newShell.setText("选择怪物组");
    }
    
    protected void buttonPressed(int buttonId) {
        if (buttonId == IDialogConstants.OK_ID) {
            StructuredSelection sel = (StructuredSelection)listViewer.getSelection();
            if (sel.isEmpty() || !(sel.getFirstElement() instanceof MonsterGroup)) {
                selectedTemplate = -1;
            } else {
                selectedTemplate = ((MonsterGroup)sel.getFirstElement()).id;
            }
        }
