 ...
 * 通用数据对象选择。
 */
public class DataObjectCellEditor extends CellEditor {
    ...
    public DataObjectCellEditor(Composite parent, Class clazz) {
    ...
    protected Control createControl(Composite parent) {
        ...
        text.addFocusListener(new FocusAdapter() {
            public void focusLost(FocusEvent e) {
                DataObjectCellEditor.this.focusLost();
            }
        });
    ...
    protected void doSetValue(Object value) {
        ...
        DataObject obj = ProjectData.getActiveProject().findObject(clazz, objectID);
        if (obj == null) {
            text.setText("无");
        } else {
    ...
    protected boolean dependsOnExternalFocusListener() {
        return getClass() != DataObjectCellEditor.class;
    }
    ...
    protected void editText() {
        ChooseDataObjectDialog dlg = new ChooseDataObjectDialog(text.getShell(), clazz, "选择");
        dlg.setSelDataObject(objectID);
        if (dlg.open() == Dialog.OK) {
            if (dlg.getSelectedDataObject() == null) {
                objectID = -1;
                text.setText("无");
            } else {
                objectID = dlg.getSelectedDataObject().id;
                text.setText(dlg.getSelectedDataObject().toString());
            }
            fireApplyEditorValue();
            deactivate();
        }
