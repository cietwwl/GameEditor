 ...
 * @author lighthu
 */
public class C_AI_CanMove extends AbstractExpr {
    ...
    public IExpr recognize(QuestInfo qinfo, Expression expr) {
        if (expr.getLeftExpr().type == Expr0.TYPE_FUNC && expr.getLeftExpr().getFunctionCall().funcName.equals("AI_CanMove")) {
            if (expr.getLeftExpr().getFunctionCall().getParamCount() != 0) {
                return null;
            }
            C_AI_CanMove ret = (C_AI_CanMove)createNew(qinfo);
