 ...
 * @author lighthu
 */
public class C_AI_CanCast extends AbstractExpr {
    ...
    public IExpr recognize(QuestInfo qinfo, Expression expr) {
        if (expr.getLeftExpr().type == Expr0.TYPE_FUNC && expr.getLeftExpr().getFunctionCall().funcName.equals("AI_CanCast")) {
            if (expr.getLeftExpr().getFunctionCall().getParamCount() != 0) {
                return null;
            }
            C_AI_CanCast ret = (C_AI_CanCast)createNew(qinfo);
