...
public class MapAIRuleInsertPointConfigPropertySource implements IPropertySource {
    ...
    public IPropertyDescriptor[] getPropertyDescriptors() {
        ...
        ret[3] = new AILogicPropertyDescriptor("怪物被移除", "怪物被移除", rule.steps[3], contextMask);
        ret[4] = new AILogicPropertyDescriptor("玩家离开地图", "玩家离开地图", rule.steps[4], contextMask);
        ret[5] = new AILogicPropertyDescriptor("检测到使用物品", "检测到使用物品", rule.steps[5], contextMask);
    ...
    public Object getPropertyValue(Object id) {
        ...
        else if ("怪物被移除".equals(id)) {
            copyVarRuleToInfo(rule.steps[3]);
            return rule.steps[3];
        }
        else if ("玩家离开地图".equals(id)) {
            copyVarRuleToInfo(rule.steps[4]);
            return rule.steps[4];
        }
        else if ("检测到使用物品".equals(id)) {
